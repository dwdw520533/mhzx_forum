...
layui.define(['layer', 'laytpl', 'form', 'element', 'upload', 'util'], function(exports){
        ...
        ,picture: function(editor){ //插入图片
          layer.open({
            ...
            ,success: function(layero, index){
              ...
              upload.render({
                elem: '#uploadImg'
                ,url: '/api/upload' //开启图床注释掉此行，并取消下一行的注释
                // ,url: 'https://sm.ms/api/upload'
                ,size: 200
                ,field: 'smfile',acceptMime: 'image/*'
                ,done: function(res){

                  // if(res.code == 'success'){
                  //   image.val(res.data.url);
                  // }
                  //开启图床注释掉这个if代码块，并取消上面的注释，保留else
                  if(res.code == 0){
                    image.val(res.data.url);
                  } else {
  ...
  var tplSigninTop = ['{{# layui.each(d.data, function(index, item){ }}'
    ,'<li>'
      ,'<a href="/user/{{item.user_id}}" target="_blank">'
        ,'<img src="{{item.user.avatar}}">'
        ,'<cite class="fly-link">{{item.user.username}}</cite>'
      ,'</a>'
      ,'<span class="fly-grey">签到于 {{ item.created_slice}}</span>'
      ,'{{# } else { }}'
  ...
  elemSigninTop.on('click', function(){
    ...
    fly.json('/api/sign/rank', function(res){ //实际使用，请将 url 改为真实接口
